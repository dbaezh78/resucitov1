...
// 3 Lista de nombres de archivos de imágenes de acordes
...
// 7 Función para obtener la configuración de posición actual basada en el ancho de la pantalla
...
// 8 Función para obtener el tipo de dispositivo basado en el ancho de la pantalla
...
// 9 Función auxiliar para parsear una sola línea de canto (letra y notas)
...
// 10 Función para parsear una sección del canto (lizq o lder), incluyendo bloques colapsables
...
// 11 Función auxiliar para renderizar una línea de canto ya parseada
...
// 12 Función para renderizar una sección específica del canto en un contenedor
...
// 13 Función principal para renderizar todo el canto
const renderCanto = () => {
...
// 14: INICIO FUNCION MODAL (Corregida para asegurar que NO guarde en 0)
const openChordSelectionModal = (currentDisplayedNoteClicked) => {
    ...
    cords.forEach(chord => {
        ...
        chordItem.addEventListener('click', async () => {
            ...
            if (idCantoActual) {
                // 2. Sincronizamos el control visual
                const elControl = document.getElementById('transporteControl');
                if (elControl) {
                    elControl.value = currentKeyOffset.toString();
                }

                // 3. PERSISTENCIA LOCAL (Indispensable para recarga)
                localStorage.setItem(`transporte_${idCantoActual}`, currentKeyOffset.toString());

                // 4. GUARDADO A LA NUBE
                // >>> CAMBIO CLAVE: Enviamos currentKeyOffset directamente como segundo argumento <<<
                if (typeof registrarCambioTecnico === 'function') {
                    await registrarCambioTecnico(idCantoActual, currentKeyOffset);
...
// 16 Nueva función para abrir la modal de imágenes de acordes
...
// 18 Función para cerrar la modal de imágenes de acordes
const closeChordImagesModal = () => {
    chordImagesModal.style.display = 'none';
};

// 19 Función para cerrar la modal de imagen grande
const closeLargeImageModal = () => {
    largeImageModal.style.display = 'none';
};

// 20 Otra funcion, poner nombre
let resizeTimeout;
window.addEventListener('resize', () => {
    ...
    resizeTimeout = setTimeout(adjustNotePositions, 100);
        ...
        // 21 Función para ajustar las posiciones de las notas después de renderizar
...
// 22 Función para renderizar las categorías como enlaces
...
// 23 Función para verificar si todos los bloques de asamblea están expandidos
...
// 24 Función para actualizar el icono del botón "Mostrar toda la asamblea"
...
// 25 Función para actualizar el icono del botón de alternar vista
...
// 26 Declaración global de currentCantoData
...
// 27 Función de inicialización que será llamada desde cada archivo de canto
const initializeCantoPage = (cantoSpecificData, processedCategories) => {
    ...
    // 27.1 Asignar referencias a los elementos del DOM
    ...
    // 27.2 Referencias para el control de audio
    ...
    // 27.3 Referencias para el nuevo botón de asamblea
    ...
    // 27.8 Referencias para los botones de navegación anterior/siguiente
    prevCantoBtn = document.getElementById('prevCantoBtn');
    nextCantoBtn = document.getElementById('nextCantoBtn');


    // 27.9 Verificar si los contenedores del canto se encontraron
    ...
    // 27.10 Actualizar los títulos y subtítulos del canto
...
if (idAudio) {
    ...
    if (audioControlBtn && cantoAudioPlayer) {
        ...
        // 27.15 Event listener para cuando el audio termina
    ...
    // 27.16 Lógica para el botón "Mostrar toda la asamblea"
    ...
    // 27.17 Lógica para el botón de alternar vista (menu_book)
    ...
    // 27.18 Lógica para el botón de mostrar imágenes de acordes (music_note)
    if (callNotasBtn) {
    ...
    // 27.19 Lógica para cerrar la modal de imágenes de acordes al hacer clic fuera
    if (chordImagesModal) {
...
// 27.20.1 Lógica para cerrar la modal de imagen grande al hacer clic fuera
    if (largeImageModal) {
...
// 27.20.2 Lógica para cerrar la modal de imagen grande al presionar 'Escape'
        document.addEventListener('keydown', (event) => {
...
// 27.24 Lógica para los botones de navegación anterior/siguiente
    ...
    // 27.25 Lógica para el modo de una sola columna
...
// FIN 27.26: LÓGICA DE PERSISTENCIA INDEPENDIENTE
    ...
    // 27.28 Event listeners para cerrar la modal de selección de acordes
    if (chordSelectionModal) {
...
// 27.29 INICIO DEL CÓDIGO A AÑADIR/MODIFICAR PARA LOS FONDOS
...
// 27.30 Si la categoría del canto existe y está definida en nuestro mapa de fondos
...
// 28 Mapa de colores para las categorías de canto, usando nombres únicos para las variables CSS estáticas
...
window.guardarCejilla = async function(cantoId, valor) {
    if (window.firebaseAPI && typeof window.firebaseAPI.guardarDato === 'function') {
        const idLimpio = cantoId.replace('.html', '');
        
        // 1. Actualizamos el valor visual en el selector
        ...
        // Actualizamos también el LocalStorage de la cejilla de inmediato
        localStorage.setItem(`cejilla_${idLimpio}`, valor);

        // 3. LLAMADA AL MOTOR UNIFICADO
        // Le pasamos el currentKeyOffset actual para que no guarde un "0" por error
        if (typeof registrarCambioTecnico === 'function') {
            // Mandamos el id y el offset que tenemos en memoria global
            await registrarCambioTecnico(idLimpio, currentKeyOffset);
            console.log(`✅ Cejilla ${valor} guardada manteniendo acorde ${currentKeyOffset}`);
        }
    }
};
...
const toggleAcordeLocation = () => {
    ...
    document.querySelectorAll('.nota-posicionada').forEach(notaSpan => {
        let display = notaSpan.querySelector('.chord-position-display');
        
        if (activo) {
            if (!display) {
                display = document.createElement('span');
                display.classList.add('chord-position-display');
                display.style.cssText = `position:absolute;top:-15px;left:50%;font-size:0.8em;color:#bc0009;font-weight:bold;transform:translateX(-50%);pointer-events:none;`;
                notaSpan.appendChild(display);
            }
            display.textContent = Math.round(parseFloat(notaSpan.style.left || 0));
            display.style.display = 'block';
            notaSpan.style.cursor = 'grab';

            // Habilitar arrastre
            if (!notaSpan.dataset.dragInit) {
                habilitarMovimientoLibre(notaSpan, display);
                notaSpan.dataset.dragInit = "true";
            }
        } else {
            if (display) display.style.display = 'none';
            notaSpan.style.cursor = 'default';
        }
    });
};

// Lógica de arrastre
function habilitarMovimientoLibre(el, display) {
    let dragging = false;
    let startX, startLeft;

    el.addEventListener('mousedown', (e) => {
        if (localStorage.getItem('pref-mant-active') !== 'true') return;
        dragging = true;
        startX = e.clientX;
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        display.textContent = Math.round(finalLeft);
    });
    ...
    document.addEventListener('mouseup', () => {
        if (dragging) {
            ...
            console.log(`Posición final: ${el.style.left}`);
        }
    });
}
